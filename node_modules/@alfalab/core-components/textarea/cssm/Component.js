...
var Textarea = React.forwardRef(function (_a, ref) {
    ...
    var _o = React.useState(false), focused = _o[0], setFocused = _o[1];
    var _p = React.useState(defaultValue || ''), stateValue = _p[0], setStateValue = _p[1];
    ...
    var handleTextareaFocus = React.useCallback(function (event) {
        setFocused(true);
        if (onFocus) {
            onFocus(event);
        }
    }, [onFocus]);
    var handleTextareaBlur = React.useCallback(function (event) {
        setFocused(false);
        if (onBlur) {
            onBlur(event);
        }
    }, [onBlur]);
    var handleTextareaChange = React.useCallback(function (event) {
        if (onChange) {
    ...
    var textareaProps = __assign(__assign({}, restProps), { className: cn__default['default'](styles__default['default'].textarea, colorStyles[colors].textarea, styles__default['default'][size], (_b = {},
            _b[styles__default['default'].hasLabel] = label,
            _b[colorStyles[colors].hasLabel] = label,
            _b[styles__default['default'].filled] = filled,
            _b[styles__default['default'].resizeVertical] = resize === 'vertical',
            _b), textareaClassName), autoComplete: autoComplete,
        disabled: disabled, onBlur: handleTextareaBlur, onFocus: handleTextareaFocus, onChange: handleTextareaChange, value: uncontrolled ? stateValue : value, rows: rows, ref: mergeRefs__default['default']([ref, textareaRef]), 'data-test-id': dataTestId, maxLength: maxLength });
    return (React__default['default'].createElement(coreComponentsFormControl.FormControl, { className: cn__default['default'](className), fieldClassName: cn__default['default'](fieldClassName, (_c = {},
            _c[styles__default['default'].focusVisible] = focusVisible,
            _c)), size: size, colors: colors, block: block, disabled: disabled, filled: filled || focused, focused: focused, error: error, label: label, hint: getHint(), leftAddons: leftAddons, rightAddons: rightAddons, bottomAddons: bottomAddons }, autosize ? (React__default['default'].createElement(TextareaAutosize__default['default'], __assign({}, textareaProps, { maxRows: maxRows, minRows: minRows, onHeightChange: onHeightChange }))) : (React__default['default'].createElement("textarea", __assign({}, textareaProps, { style: { maxHeight: maxHeight } })))));
});
/**
