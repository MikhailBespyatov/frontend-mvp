...
var setGradientCharts = function (series) {
    ...
    return filterSeries.reduce(function (accum, item) {
        ...
        if (chart === 'area') {
            ...
            accum.push(__assign(__assign({}, item), { zIndex: -100, chart: 'gradient', hideLegend: true, hideTooltip: true, properties: __assign(__assign({}, item.properties), { dataKey: item.properties.dataKey + "-gradient", fill: fill }), data: newData || item.data }));
        }
        accum.push(item);
        return accum;
    }, []);
