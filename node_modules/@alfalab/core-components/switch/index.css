...
.switch__center_j1p4y {
    align-items: center;
}
.switch__addons_j1p4y {
    margin-left: auto;
...
.switch__label_j1p4y:not(:only-child) {
...
/* Reversed state */
.switch__component_j1p4y.switch__reversed_j1p4y {
    flex-direction: row-reverse;
}
.switch__reversed_j1p4y .switch__content_j1p4y {
    margin-right: var(--gap-m);
    margin-left: 0;
}
.switch__reversed_j1p4y .switch__addons_j1p4y {
    margin-left: 0;
    padding-left: 0;
    margin-right: auto;
...
.switch__checked_j1p4y .switch__switch_j1p4y:before {
    /* ширина компонента(36px + 2*2px) - отступы(2 * 2px) - размер кружка(20px) */
    transform: translateX(16px);
}
/* Disabled state */
.switch__disabled_j1p4y {
