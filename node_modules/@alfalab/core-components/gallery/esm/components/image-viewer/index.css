...
.gallery__singleSlide_xiewl .gallery__swiper_xiewl {
    max-height: calc(100vh - 80px);
    padding: var(--gap-2xl);
}
.gallery__hidden_xiewl {
    display: none;
}
.gallery__slide_xiewl {
    position: relative;
    display: flex;
    justify-content: center;
    align-items: center;
...
.gallery__verticalImageFit_xiewl {
    width: auto;
    height: 100%;
}
.gallery__horizontalImageFit_xiewl {
    width: 100%;
    height: auto;
}
...
.gallery__brokenImgWrapper_xiewl {
    position: relative;
    display: flex;
    flex-direction: column;
    align-items: center;
